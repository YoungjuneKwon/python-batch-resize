...
def resize_and_crop(org, dst, size):
  ...
  i2 = restore_rotation(img.resize(s1).crop((d[0], d[1], s1[0] - d[0], s1[1] - d[1])).resize(size))
  i2.save(validate_path(dst), 'JPEG', optimize=True, exif=img.info['exif'], icc_profile = img.info.get('icc_profile',''), subsampling=JIP.get_sampling(img))
  return i2